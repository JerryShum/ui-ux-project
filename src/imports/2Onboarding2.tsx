import svgPaths from "./svg-d2aolgu9wk";
import imgCopperAndWildYf61JhXNmYoUnsplash1 from "figma:asset/373dc965b73114bcff9d7aaaa4d4212dcc9011a5.png";
...
function Content() {
  return (
    <div className="absolute bg-white box-border content-stretch flex flex-col gap-[20px] items-center left-0 overflow-clip px-[10px] py-[40px] rounded-[20px] size-[440px] top-[516px]" data-name="Content">
      <div className="h-[47px] relative shrink-0 w-[50px]" data-name="􀌴">
        <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 50 47">
          <path d={svgPaths.pc090400} fill="var(--fill-0, #0077CA)" id="ô´" />
        </svg>
      </div>
      <div className="capitalize flex flex-col font-['Inter:Bold',sans-serif] font-bold justify-center leading-[0] min-w-full not-italic relative shrink-0 text-[#0077ca] text-[24px] text-center w-[min-content]">
        <p className="leading-[normal]">{`Connect & Trade Securely`}</p>
      </div>
      <div className="flex flex-col font-['Inter:Medium',sans-serif] font-medium justify-center leading-[0] not-italic relative shrink-0 text-[14px] text-black text-center w-[370px]">
        <p className="leading-[normal]">Our built-in messaging allows you to communicate directly and safely with others. Arrange meetups and finalize deals knowing your transactions are secure.</p>
      </div>
      <PageControl />
      <Button />
    </div>
...
export default function Component2Onboarding() {
  return (
    <div className="bg-white overflow-clip relative rounded-[20px] size-full" data-name="2 - Onboarding 2">
      <div className="absolute left-[-76px] size-[572px] top-[9px]" data-name="copper-and-wild-yf61jhXNmYo-unsplash 1">
        <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgCopperAndWildYf61JhXNmYoUnsplash1} />
      </div>
      <Content />
      <TopIcons />
    </div>
